...
extern "C" {
    fn host_truapi_send(pointer: u32, length: u32) -> u32;
    fn host_truapi_poll(pointer: u32, capacity: u32) -> i32;
    fn host_save_submit(pointer: u32, length: u32) -> u32;
}

#[polkavm_derive::polkavm_export]
extern "C" fn init() {
    let status = unsafe { host_truapi_send(REQUEST.as_ptr() as u32, REQUEST.len() as u32) };
    assert_eq!(status, 0);
}
